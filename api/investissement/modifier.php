...
try {
    ...
    file_put_contents($logDir . "/api_debug.log", 
        date('Y-m-d H:i:s') . " - MODIFIER - " . print_r($data, true) . "\n", 
        FILE_APPEND);
    ...
    if ($success) {
        ...
        // Tell the user
        echo json_encode(["success" => false, "message" => "Impossible de modifier le projet"]);
    }
} catch (Exception $e) {
    // Log error
    file_put_contents($logDir . "/api_errors.log", 
        date('Y-m-d H:i:s') . " - MODIFIER - Error: " . $e->getMessage() . "\n", 
        FILE_APPEND);
